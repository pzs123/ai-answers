...
public class QuestionController {
    ...
    public BaseResponse<List<QuestionContentDTO>> aiGenerateQuestion(@RequestBody AiGenerateQuestionRequest aiGenerateQuestionRequest) {
        ...
        // AI 生成
        String result = aiManager.doRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage, false, 0.05f);
        // 结果处理
...
//                            System.out.println(JSONUtil.toJsonStr(contentBuilder.toString()));
...
//    }
    ...
    public SseEmitter aiGenerateQuestionSSE(AiGenerateQuestionRequest aiGenerateQuestionRequest) {
        ...
        // AI 生成, SSE 流式返回
        Flowable<ModelData> modelDataFlowable = aiManager.doStreamRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage, null);
        // 左括号计数器，除了默认值外，当回归为 0 是，表示左括号等于右括号，可以截取
                    ...
                    {                    // 如果是 '{'，计数器 + 1
                        ...
                        if (c == '}') {
                            ...
                            if (counter.get() == 0) {
                                ...
                                System.out.println(JSONUtil.toJsonStr(stringBuilder.toString()));
