package com.pzs.panpan.service.impl;
...
import com.pzs.panpan.constant.CommonConstant;
import com.pzs.panpan.exception.ThrowUtils;
import com.pzs.panpan.mapper.ScoringResultMapper;
import com.pzs.panpan.model.dto.scoringResult.ScoringResultQueryRequest;
import com.pzs.panpan.model.entity.App;
import com.pzs.panpan.model.entity.ScoringResult;
import com.pzs.panpan.model.entity.User;
import com.pzs.panpan.model.vo.ScoringResultVO;
import com.pzs.panpan.model.vo.UserVO;
import com.pzs.panpan.service.AppService;
import com.pzs.panpan.service.ScoringResultService;
import com.pzs.panpan.service.UserService;
import com.pzs.panpan.utils.SqlUtils;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(StringUtils.isBlank(resultProp), ErrorCode.PARAMS_ERROR, "结果属性集合不能为空");
            ThrowUtils.throwIf(resultScoreRange == null, ErrorCode.PARAMS_ERROR, "结果得分范围非法");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用id非法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 128, ErrorCode.PARAMS_ERROR, "标题名称长度不能超过128");
        }
        if (appId != null) {
